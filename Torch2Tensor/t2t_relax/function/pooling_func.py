...
class Pool2dFunc(BaseLayer):
    ...
    def get_avgpool2d_attr(self):
        ...
        if func_name.startswith('adaptive'):
            attr_dict = dict(
                output_size = self.get_value_by_key_or_index("output_size", 1, None),
                layout = "NCHW",
                out_layout = None,
            )
            ...
            attr_dict = dict(
                pool_size = self.get_value_by_key_or_index("kernel_size", -1, 1),
                strides = self.get_value_by_key_or_index("stride", -1, 1),
                padding = self.get_value_by_key_or_index("padding", -1, 0),
                dilation = self.get_value_by_key_or_index('dilation', -1, 1),
                ceil_mode = self.get_value_by_key_or_index('ceil', -1, False),
                layout = "NCHW",
                out_layout = "NCHW",
            )
